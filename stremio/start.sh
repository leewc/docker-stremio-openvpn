...
# --------------------------------------------------------------------------
# Taken from /etc/transmission/userSetup.sh
RUN_AS=root
...
echo "
-------------------------------------
Stremio client/server will run as
-------------------------------------
User name:   ${RUN_AS}
...
echo "STARTING STREMIO SERVER at port 11470 (HTTP) and port 12470 (HTTPS)..."
exec su --preserve-environment ${RUN_AS} -s /bin/bash -c "HOME=/config/ PATH=/usr/local/sbin/:$PATH /usr/local/bin/node /stremio/server.js" &
...
echo "STARTING STREMIO STATIC WEB CLIENT at port 8080"
exec su --preserve-environment ${RUN_AS} -s /bin/bash -c "python3 -m http.server 8080 --directory /stremio/ui" &

echo "Stremio startup script complete."

# If transmission-post-start.sh exists, run it
if [[ -x /scripts/stremio-post-start.sh ]]; then
  echo "Executing /scripts/stremio-post-start.sh"
  /scripts/stremio-post-start.sh "${USER_SCRIPT_ARGS[@]}"
  echo "/scripts/stremio-post-start.sh returned $?"
fi
