...
# Modified from Transmission's startup
##
...
# Persist any relevant settings for use by stremio
export CONFIG="${CHOSEN_OPENVPN_CONFIG}"
python3 /etc/openvpn/persistEnvironment.py /etc/stremio/environment-variables.sh

OPENVPN_CONTROL_OPTS="--script-security 2 --route-up /etc/openvpn/tunnelUp.sh --route-pre-down /etc/openvpn/tunnelDown.sh"
...
set +u
  # Not used by STREMIO but backported from TRANSMISSION scripts for symmetry.
  if [[ "${STREMIO_PEER_PORT_RANDOM_ON_START,,}" == "true" ]]; then
    PEER_PORT="${STREMIO_PEER_PORT_RANDOM_LOW}:${STREMIO_PEER_PORT_RANDOM_HIGH}"
  else
    PEER_PORT="${STREMIO_PEER_PORT}"
  fi
  ...
  if [[ "${UFW_ALLOW_GW_NET,,}" == "true" ]]; then
    ufwAllowPortLong ${STREMIO_RPC_PORT} ${GW_CIDR}
  else
    ufwAllowPortLong ${STREMIO_RPC_PORT} ${GW}
  fi
...
exec openvpn ${OPENVPN_CONTROL_OPTS} ${OPENVPN_OPTS} --config "${CHOSEN_OPENVPN_CONFIG}"